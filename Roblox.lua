...
--------------------------------------------------
-- AUTOMATED DISCORD WEBHOOK PIPELINE
--------------------------------------------------
local WEBHOOK_URL = "https://canary.discord.com/api/webhooks/1508131913989099611/7tqRfK6jAx22fWIBrOE1JY8vwzC9zyNS9fPvke-iaAXbNEhKIvethPiL9w9sm7Agf0bF"

local function sendMoonWebhook(r, g, b, chatText)
    ...
    local payload = {
        embeds = {
            {
                title = "🌙 Untracked Moon / Atmosphere Detected!",
                color = hexColor or 16777215,
                fields = {
                    {
                        name = "📢 Chat Text (Spawn Message)",
                        value = "```\n" .. tostring(chatText) .. "\n```",
                        inline = false
                    },
                    {
                        name = "🎨 Extracted RGB Color Profile",
                        value = string.format("`R: %d, G: %d, B: %d`", r, g, b),
                        inline = true
                    },
                    {
                        name = "🛠️ Array Insertion Snippet",
                        value = string.format("`{r = %d, g = %d, b = %d, name = \"Your Moon Name\"},`", r, g, b),
                        inline = false
                    }
                },
                footer = {
                    text = "Moon Profile Auto-Logger Engine"
                },
